...
class Session {
	...
	protected function _ci_load_classes_legacy($driver)
	{
		...
		$prefix = config_item('subclass_prefix');
 		...
 		{
			if (file_exists($file_path = resolve_path(APPPATH,'libraries').'/Session/Session_driver.php'))
 			{
 				require_once($file_path);
 			}

 			if (file_exists($file_path = resolve_path(APPPATH,'libraries').'/Session/'.$prefix.'Session_driver.php'))
 			{
 				require_once($file_path);
 			}
 		}
		...
		if ( ! class_exists($class, FALSE) && file_exists($file_path = resolve_path(BASEPATH,'libraries').'/Session/Drivers/'.$class.'.php'))
		{
			require_once($file_path);
		...
		{
			if (file_exists($file_path = resolve_path(APPPATH,'libraries').'/Session/Drivers/'.$class.'.php') OR file_exists($file_path = BASEPATH.'libraries/Session/Drivers/'.$class.'.php'))
			{
				require_once($file_path);
			}
		...
		if ( ! class_exists($prefix.$class, FALSE) && file_exists($file_path = resolve_path(APPPATH,'libraries').'/Session/Drivers/'.$prefix.$class.'.php'))
		{
			require_once($file_path);
