...
class Blade{
    ...
    protected function _compile_yields($value) {
        ...
        return preg_replace_callback($pattern, function ($matches) {
            // Memastikan kita menangkap bagian yang benar
            if (preg_match('/\(\s*\'([^\']+)\'\s*,\s*([^\)]+)\s*\)/', $matches[2], $params)) {
                // $params[1] adalah nama section (misalnya 'version')
                // $params[2] adalah konten default (misalnya $version)
                $section = $params[1];
                $default = $params[2];

                // Mengecek apakah section dengan nama tertentu sudah ada
                return "<?php echo isset(\$this->_sections['$section']) ? \$this->_sections['$section'] : $default; ?>";
            }

            // Jika tidak ada parameter yang cocok, tetap menggunakan metode default
    ...
    protected function _compile_section_start($value) {
        ...
        return preg_replace_callback($pattern, function ($matches) {
            // Memastikan kita menangkap bagian yang benar
            if (preg_match('/\(\s*\'([^\']+)\'\s*,\s*([^\)]+)\s*\)/', $matches[2], $params)) {
                // $params[1] adalah nama section (misalnya 'title')
                // $params[2] adalah konten (misalnya kodhe('setup')->get('name'))
                $section = $params[1];
                $content = $params[2];

                // Memeriksa apakah konten tampaknya adalah ekspresi PHP atau bukan
                // Mengecek apakah konten mengandung karakter kurung atau operator yang menandakan ekspresi PHP
                if (preg_match('/[\(\)\->\[]/', $content)) {
                    // Jika mengandung karakter seperti (), ->, atau [], anggap sebagai ekspresi PHP
                    return "<?php \$this->_section_start('$section'); echo $content; \$this->_section_end(); ?>";
                } else {
                    // Jika tidak, anggap sebagai string literal
                    return "<?php \$this->_section_start('$section'); echo $content; \$this->_section_end(); ?>";
                }
            }
